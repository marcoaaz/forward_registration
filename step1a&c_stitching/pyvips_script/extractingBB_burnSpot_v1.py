# -*- coding: utf-8 -*-
"""
Extracting image patches from a very large image using bounding box coordinates and watershed segmentation masks.
The microanalytical spots are read from the Master table and burned on the patches.
Written looking forward a collaboration with Iolite developer Joe Petrus.

The latest output = E:\Feb-March_2024_zircon imaging\cl_zircon_twoCorrections\originals_fixed\input_v5\Cathodoluminescence
...
#automatic input
image_name = "Cathodoluminescence" #other modalities
root_dir = r"E:\Feb-March_2024_zircon imaging\cl_zircon_twoCorrections\originals_fixed"

workingDir = r"E:\Feb-March_2024_zircon imaging\cl_zircon_twoCorrections\originals_fixed\input_v5"
tableDir = r"E:\Feb-March_2024_zircon imaging\05_zircon geochemical data\processed_files"
table_name = "all_appended_John.xlsx"

## Script

table1 = pd.read_excel(os.path.join(tableDir, table_name)) #master table
table_paths = table1["file_paths"].apply(os.path.basename)
# table_paths = os.path.basename(temp)
# print(temp)

maskDir = os.path.join(workingDir, "Binary masks") #real size masks

#Edit to have a different input (all images must be registered with binary masks)
path_list = glob.glob(f"{root_dir}/*.tif", recursive = True) + glob.glob(f"{root_dir}/*.png", recursive = True)
#path_list = [os.path.join(root_dir, 'ECU_996_997_CL_20X_rl_registered.tif')]
...
for imageInput_path in path_list:    
    ...
    #for filename in fileList[499:501]: #trial 1 (should not save table)
    #for filename in fileList[499:610]: #trial 1 (should not save table)
        ...
        condition = (match_original.group(1) in tag) 
        #patch sample has matching large image (e.g., ECU_996_997_CL_20X_xpl_registered.tif)
        ...
        if match and condition:                  
            ...
            #matched microanalytical spot coordinates (backwards registration)
            # a = os.path.basename(filename)
            patch_name = f"{patch}_[x={x},y={y},w={w},h={h}].tif"
            condition2 = table_paths == patch_name                        
            
            if any(condition2):
                table2 = table1.loc[condition2, :]            
                spot_size = table2["Spot Size"].to_numpy(dtype=float)
                x_spot = table2["X_reg"].to_numpy(dtype=float)
                y_spot = table2["Y_reg"].to_numpy(dtype=float)                

                #generating spot mask
                centre_x = x_spot - x
                centre_y = y_spot - y
                radius = spot_size/2  
                thick = 100 
            ...
            xx, yy = np.mgrid[:h, :w]
            circumference = (xx - centre_x) ** 2 + (yy - centre_y) ** 2
            circle = ( circumference < (radius**2 + thick) ) & ( circumference > (radius**2 - thick) )
            circle2 = pyvips.Image.new_from_array(circle)
            mask_circle = (circle2 == 0).bandand()

            #getting original binary mask
            image = pyvips.Image.new_from_file(filename) #cannot be "sequential" access
            mask = (image == 0).bandand() #background=1            

            #generating squared patch (enlarged ROI)            
            bb = [x-2, y-2, w+4, h+4]
            height_factor = bb[3] / mask.height
            width_factor = bb[2] / mask.width
            mask_enlarged = mask.affine((width_factor, 0, 0, height_factor))
            mask_circle_enlarged = mask_circle.affine((width_factor, 0, 0, height_factor))

            #extracting patch from large image
            tile = large_image.crop(x, y, w, h) #for stats
            tile_enlarged = large_image.crop(bb[0], bb[1], bb[2], bb[3]) #for patch            
            tile_enlarged = tile_enlarged.cast("uchar", shift= True) #optional = 16-bit to 8-bit

            #extracting object
            tile_masked = mask.ifthenelse(0, tile) #where bg=1, keep background. Used for descriptive statistics.
            tile_masked_enlarged = mask_enlarged.ifthenelse(0, tile_enlarged) #for classification
            
            #blending spot
            tile_masked_enlarged2 = mask_circle_enlarged.ifthenelse(tile_masked_enlarged, [255, 0, 0]) #
            ...
            img_zero = img_zero.insert(tile_masked_enlarged2, 
                           start_point_x, #top
                           start_point_y) #left   
