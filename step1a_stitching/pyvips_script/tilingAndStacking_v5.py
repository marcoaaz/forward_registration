...
'stackingCPS_synchrotron_v6.m’ in Acevedo Zamora et al. (2024). 

The update follows 'stitch_stretch_batch.py' (Acevedo Zamora and Kamber, 2023) in
using the pyvips library
...
Updated:
second version for whole-slide imaging: 19-Jun-24
minor adjustments: 5-Sep-24
...
imageFolder = r"E:\Teresa_article collab\91702_80R6w\tiff"
sourceFolder = os.path.split(imageFolder)

fileList = glob.glob(f"{imageFolder}/91702-*.tiff") #perfect match needed with 'pattern'
pattern = re.compile(r".+\\91702-(.+)\.tiff") 
...
trial_name = 'trial2'
pctOut = 0.5 #percentile out in the input (for colour contrast)
...
flip_ud = 0 #whether to flip outputs upside down
...
#for filename in fileList[0:3]: #trial
    ...
    if match:                
        ...
        # print(f"Processing experiment match {k}: {element}")         
...
stats_table.insert(0, "experiment", experiment_list, True) #optional
